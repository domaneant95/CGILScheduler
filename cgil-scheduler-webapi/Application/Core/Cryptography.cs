...
namespace Application.Core
{
    ...
    public class RSAParameters
    {
        ...
        public System.Security.Cryptography.RSAParameters PublicParameters { get; set; }
        public System.Security.Cryptography.RSAParameters PrivateParameters { get; set; }
    ...
    public static class Cryptografy
    {
        ...
        public class RSA
        {
            ...
            public bool DoOAEPPAdding { get; private set; } = true;
            public int MaxNumberOfBytesToBeEncrypted { get; private set; }

            private readonly RSACryptoServiceProvider rsa;

            public RSA(int keySize, System.Security.Cryptography.RSAParameters rsaParameters, bool doOAEPPAdding = true)
            {
                ...
                DoOAEPPAdding = doOAEPPAdding;
            }

            public string Encrypt(string text)
            {
                return Convert.ToBase64String(Encrypt(UTF8Encoding.UTF8.GetBytes(text)));
            }

            public string Decrypt(string base64)
            {
                return UTF8Encoding.UTF8.GetString(Decrypt(Convert.FromBase64String(base64)));
            }
            ...
            public static RSAParameters CreateParametersAndKeys(int keySize)
            {
                var rsaParametersKeys = new RSAParameters();

                var provider = new RSACryptoServiceProvider(keySize);

                rsaParametersKeys.PublicParameters = provider.ExportParameters(false);
                rsaParametersKeys.PrivateParameters = provider.ExportParameters(true);

                rsaParametersKeys.PublicKey = ComposePublicKeyFromRSAProvider(provider);

                return rsaParametersKeys;
            }
            ...
            public static string ComposePublicKeyFromRSAProvider(RSACryptoServiceProvider csp)
            {
                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    int offset = 0;
                    var base64 = Convert.ToBase64String(stream.GetBuffer(), offset, (int)stream.Length - offset).ToCharArray();
                ...
                var publicKey = outputStream.ToString();
...
/*public class PemKeyUtils
{
    ...
    //    const String pemp8footer = "-----END PRIVATEKEY-----";
